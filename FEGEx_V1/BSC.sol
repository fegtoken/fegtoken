...
/*
* Must wrap your BNB for fBNB to use FEGex DEX

Built for fBNB - FEG Wapped BNB - Built in 1% frictionless rewards of BNB!  Stake BNB with fBNBand earn rewards!
*/
...
contract FToken is ReentrancyGuard, FTokenBase {
    ...
    string  private _name     = "FEGwBNBpair";
    string  private _symbol   = "FEGwBNBLP";
...
contract FEGwBNB is FSilver, ReentrancyGuard, FToken, FMath {
    ...
    function AddfBNBInitial(address token, uint balance, uint denorm)
        external
        _logs_
        // _lock_  Bind does not lock because it jumps to `rebind`, which does
    {
        ...
        _lock_
    {
        ...
        for (uint i = 0; i < _tokens.length; i++) {
            ...
            _records[fBNB].balance = IERC20(fBNB).balanceOf(address(this));
        }
        ...
        returns (uint tokenAmountOut, uint spotPriceAfter)
    {
        ...
        require(tokenIn == fBNB, "Can only buy with fBNB");
        require(_records[tokenIn].bound, "ERR_NOT_BOUND");
        require(_records[tokenOut].bound, "ERR_NOT_BOUND");
    ...
    function addLiquidityfBNB(address tokenIn, uint tokenAmountIn, uint minPoolAmountOut)
        external
        _logs_
        _lock_
    ...
    {
        require(tokenIn == fBNB, "Can only add fBNB");
