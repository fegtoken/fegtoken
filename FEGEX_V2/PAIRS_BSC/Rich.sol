...
contract FConst is FSilver, ReentrancyGuard {
    ...
    uint public  SM = 10;
    address public FEGstake = 0xD315e769c95A79563Acf4F2Cff620D76aa02e030;
...
contract FToken is ReentrancyGuard, FTokenBase {
    ...
    string  private _symbol   = "RICHfBNB";
...
contract FEGexV2 is FSilver, ReentrancyGuard, FToken, FMath {
    ...
    address private _poolOwner = 0x0bc15279686C07C55F09B440D55C57456BED7B1B;
    address public Wrap = 0x87b1AccE6a1958E522233A737313C086551a5c76;
    address public Token = 0xC7Bc24c4C18F8251D31611114d0e7B5F5ef76762;
    address public pairRewardPool = 0xe90944d678b2fB17dB5e9d284282FAAC8Dde7dED;
    address public burn = 0x000000000000000000000000000000000000dEaD;
    uint public FSS = 25; // FEGstake Share
    uint public PSS = 20; // pairRewardPool Share 
    uint public RPF = 995; //Smart Rising Price Floor Setting
    uint public SHR = 1000; //p2p fee Token
    ...
    function depositToken(uint256 amt)  external noContract nonReentrant {
        ...
        uint256 finalAmount = amt * 95/100;
        ...
        _lock_
    {
        ...
        for (uint i = 0; i < _tokens.length; i++) {
            ...
            emit LOG_JOIN(msg.sender, t, tokenAmountIn * 97/100, 0);
        ...
        returns (uint tokenAmountOut, uint spotPriceAfter)
    {
        ...
        emit LOG_SWAP(msg.sender, tokenIn, tokenOut, msg.value * 99/100, tokenAmountOut * 95/100);
        return (tokenAmountOut, spotPriceAfter);
    }
        ...
        returns (uint tokenAmountOut, uint spotPriceAfter)
    {
        ...
        (tokenAmountOut, tokenInFee) = calcOutGivenIn(
                                            ...
                                            outRecord.balance,
                                            BASE * 25,
                                            tokenAmountIn * 95/100,
                                            _swapFee
                                        );
        require(tokenAmountOut >= minAmountOut, "ERR_LIMIT_OUT");
        
        emit LOG_SWAP(msg.sender, tokenIn, tokenOut, tokenAmountIn * 95/100, tokenAmountOut * 99/100);
